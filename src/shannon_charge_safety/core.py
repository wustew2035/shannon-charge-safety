...
def surface_area_mm2_from_width_height(width_mm: float, height_mm: float) -> float:
    """Return rectangular electrode surface area in mm^2."""
    width_mm = _require_positive("width_mm", width_mm)
...
def calculate_charge_safety(
    *,
    ...
    2. width_mm=<width in mm> and height_mm=<height in mm>
    """
    has_area = area_mm2 is not None
    has_dimensions = width_mm is not None or height_mm is not None

    if has_area and has_dimensions:
        raise ValueError("Provide either area_mm2 OR width_mm and height_mm, not both")
    if not has_area and not has_dimensions:
        raise ValueError("Provide area_mm2 OR width_mm and height_mm")
    if has_dimensions and (width_mm is None or height_mm is None):
        raise ValueError("Both width_mm and height_mm are required when using dimensions")

    if area_mm2 is None:
        area_mm2 = surface_area_mm2_from_width_height(width_mm, height_mm)  # type: ignore[arg-type]
    else:
        area_mm2 = _require_positive("area_mm2", area_mm2)
